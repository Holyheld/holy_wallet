...
import ClaimInfo from '../components/treasury-bank/ClaimInfo';
import exchangeModalTypes from '../helpers/exchangeModalTypes';
import useTreasuryClaimInputs from '../hooks/useTreasuryClaimInputs';
import { loadWallet } from '../model/wallet';
import { useNavigation } from '../navigation/Navigation';
import useStatusBarManaging from '../navigation/useStatusBarManaging';
import createClaimTreasuryCompoundRap, {
  estimateClaimTreasuryCompound,
} from '../raps/claimTreasuryCompound';
...
const TreasuryBurnModal = ({ balance, testID }) => {
  ...
  const createRap = createClaimTreasuryCompoundRap;
  const estimateRap = estimateClaimTreasuryCompound;
  const type = exchangeModalTypes.treasuryClaim;
  const inputCurrency = {
    ...
    backgroundTask.execute(async () => {
      ...
      try {
        ...
        if (!wallet) {
          ...
          logger.sentry(`aborting treasury claim due to missing wallet`);
          return;
        }
        ...
        logger.log('[treasury claim] rap', rap);
        await executeRap(wallet, rap);
        logger.log('[treasury claim] executed rap!');
      } catch (error) {
        setIsAuthorizing(false);
        logger.log('[treasury claim] error submitting claim', error);
        setParams({ focused: false });
        navigate(Routes.WALLET_SCREEN);
      }
    });
